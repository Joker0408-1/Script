...
if (Eecrypt) {
    ...
    encrypted = fs.readFileSync('./vediock.txt', 'utf8');
    decrypted = aesDecrypt(encrypted, key);
    vediock = JSON.parse(decrypted);
...
vediockArr=vediock.watchvediocks.split("&&");
...
async function all() {
    ...
    for (let i = 6; i < Length; i++) {
		...
		for (let j = 0; j < vediockArr.length; j++) {
			vediockArr2=vediockArr[j].split('&');
			await watchvedio(i);
		}
        return;
    }
...
function singlelive(i) {
    return new Promise((resolve) => {
        ...
        O = (`${$.name + (i + 1)}🔔`);
        header = JSON.stringify(iboxpayheaderVal).replace(`${token}`, `${TOKEN[i]}`).replace(`${oldtime}`, `${tts()}`);
        livesbodyVal = `{"actId": "${zbid[0].actId}","liveId": "${liveId[0][` $ {
            IDNum
        }
`]}"}`;
        ...
        $.post(url, async(err, resp, data) => {
            try {
                ...
                if ($.lives.resultCode == 1) {
                    ...
                    console.log(`【直播奖励】${$.name+(i+1)}领取第${parseInt(` $ {
                        IDNum
                    }
...
function watchvedio(i) {
    return new Promise((resolve, reject) => {
        ...
        headers = JSON.parse(vediockArr2[1]);
		headers["token"]=`${TOKEN[i]}`;
        ...
        $.post(request, async(error, response, data) => {
            try {
				 ...
				 {
					 console.log(`【视频奖励】${$.name+(i+1)}领取 ` +result.data.goldCoinNumber + '金币'+',等待30秒继续观看视频');					 
					 await $.wait(30000);
					// await watchvedio(i);
				 }
				 else
				 {
					 await console.log('【视频奖励】观看视频失败,错误代码：'+result.errorCode);
