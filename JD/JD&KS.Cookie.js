...
京东Cookie：
  ...
  ^https?://plogin\.m\.jd\.com/cgi-bin/m/wxlogincenter\? url script-request-header NE.cookie.js
...
!(async () => {
  if ($.isRewrite && $request.method != 'OPTIONS') {
    ...
    oldCks.forEach(item => ckArr.push(item.cookie || `pt_pin=${item.userName};`));
    ...
    if (result < 0) {
      ...
      for (let i = 0, len = ckArr.length - 1; i <= len; i++) {
        newCks.push({
          ...
          cookie: ckArr[i].match(/pt_key=(.+?);/) ? ckArr[i] : ''
        });
      }
      for (let i = 0; !$.jdCkFlag && i < 2; i++) {
        let ck = '';
        if (newCks.length > 0) {
          ck = newCks[0];
          newCks.splice(0, 1);
  ...
  return new Promise(resolve => {
    try {
      ...
      if ($.isResponse && $response.headers) {
        ...
        let mArr = ck.match(/EXPIRES=(.+?GMT)/g) || []
        if (mArr.length > 0) {
          let expMap = {}
          mArr.map(s => s.substr(8)).forEach(s => {
            if (!expMap[s]) {
              expMap[s] = new Date(s).getTime()
            }
          })
          for (let k in expMap) {
            ck = ck.replaceAll(k, expMap[k])
          }
          mArr = ck.split(/,/).filter(s => /pt_key=.+EXPIRES=\d+/.test(s))
          if (mArr.length > 0) {
            ck = ck + '; jd_key_exp=' + mArr[0].match(/EXPIRES=(\d+)/)[1]
          }
        }
        extMsg = ck
      ...
      if (ck) {
        ...
        if (ckItems.length == 2) {
          ...
          let exp = (ck.match(/jd_key_exp=(\d+)/) || ['', ''])[1]
          if (exp) {
            acObj.exp = $.time('yyyy-MM-dd HH:mm:ss', new Date(exp - 0))
          }
        }
        ...
        if (!acObj.cookie) {
          ...
          const ckArr = $.jdCkFlag ? [] : [$.getdata('CookieJD'), $.getdata('CookieJD2')];
          ...
          if (status) {
            if (status > 0) {
              let wt = '';
              if (!$.jdCkFlag && seatNo < 2) {
                wt = $.setdata(acObj.cookie, `CookieJD${$.suffix(seatNo)}`);
              } else {
                let idx = $.jdCkFlag ? seatNo : seatNo - 2;
                // 更新，需判断能否比较过期时间
                let oldTime = (oldCks[idx] && oldCks[idx].exp) || $.time('yyyy-MM-dd HH:mm:ss', new Date().getTime() + 30 * 24 * 60 * 60 * 1000)
                let newTime = acObj.exp || $.time('yyyy-MM-dd HH:mm:ss', new Date().getTime() + 30 * 24 * 60 * 60 * 1000)
                if (status == 1 || oldTime <= newTime) {
                  oldCks[idx] = {...oldCks[idx], ...acObj};
              ...
              $.msg($.name, `${appName} ${seatNo+1}: ${acObj.userName}`, `${status==1?'新增':'更新'}京东Cookie${wt?`成功 🎉`:`失败 ‼️`}${extMsg}`);
            } else {
              $.log($.name, `${appName} ${seatNo+1}: ${acObj.userName}`, `Cookie数据已存在，跳过处理${extMsg}`);
            }
          }
        }
      }
